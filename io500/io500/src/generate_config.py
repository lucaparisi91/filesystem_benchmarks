...
def create_directories(settings):
    ...
    subprocess.run( ["rm","-rf",dataDir] )
    subprocess.run( ["mkdir","-p",dataDir] )
    if settings["striped"] == True :
        commands=["lfs","setstripe","-c","-1"]
        if "pool" in settings.keys():
            commands=commands + ["-p",settings["pool"]]    
        subprocess.run([*commands,dataDir])
